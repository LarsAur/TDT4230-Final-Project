...
int main()
{
    ...
    Portal bluePortal(glm::vec2(5, 10));
    Portal orangePortal(glm::vec2(5, 10));
    ...
    root.addChild(&bluePortal);
    root.addChild(&orangePortal);

    Texture portalGunAlbedo("../res/textures/portalgun_col.jpg");
    Texture wall("../res/textures/wall.png");
    Texture bluePortalTexture("../res/textures/blue_portal.png");
    Texture orangeportalTexture("../res/textures/orange_portal.png");
    Texture rubix("../res/textures/rubix.jpg");
    Texture portalStencil("..res/textures/portal_stencil.png");

    portalGun.albedo = &portalGunAlbedo;
    cube.albedo = &rubix;
    chamber.albedo = &wall;
    bluePortal.albedo = &bluePortalTexture;
    orangePortal.albedo = &orangeportalTexture;

    chamber.generateVertexData(&shader);
    cube.generateVertexData(&shader);
    portalGun.generateVertexData(&shader);
    bluePortal.generateVertexData(&shader);
    orangePortal.generateVertexData(&shader);

    portalGun.setPosition(glm::vec3(0.007f, -0.005f, -0.01f));
    portalGun.setRotation(glm::vec3(0, -0.3f, 0.1f));

    bluePortal.translate(glm::vec3(0, 0, -4.5f));
    orangePortal.translate(glm::vec3(10, 0, 0));
    ...
    while (!window.shouldClose())
    {
        ...
        bluePortal.setPosition(glm::vec3(0, 2 * sin(t), -4.5f));
        cube.rotate(glm::vec3(0,0.01f, 0));
        ...
        auto renderPortal = [&](Portal src, Portal dest)
        {
            int uPortalRenderLoc = shader.getUniformLocation("render_portals");
            glEnable(GL_STENCIL_TEST);
            ...
            glClear(GL_STENCIL_BUFFER_BIT);
                ...
                glUniform1i(uPortalRenderLoc, 1);
                src.render();
                glUniform1i(uPortalRenderLoc, 0);
            ...
            glStencilFunc(GL_LEQUAL, 1, 0xFF);
                ...
                glUniformMatrix4fv(uViewLoc, 1, GL_FALSE, glm::value_ptr(src.getViewMatrix(view, &dest)));
                renderWorld();
        ...
        renderPortal(bluePortal, orangePortal);
        
        renderPortal(orangePortal, bluePortal);
